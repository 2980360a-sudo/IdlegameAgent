...
用法:
  python main.py --game melvor --mode auto
  python main.py --game melvor --mode inspect
  python main.py --game melvor --mode guards
  python main.py --config config/rules/melvor_idle.yaml
"""
import asyncio
import argparse
import json
...
from core.safety import dismiss_post_load_modals
from adapters.melvor_idle import MelvorIdleAdapter


async def run_agent(game_name: str, mode: str = 'auto', config_path: str = None):
    """运行指定游戏的 Agent。
    mode: auto | inspect | guards | manual
    """
    log(f'===== IdleAgent 启动 | 游戏: {game_name} | 模式: {mode} =====')

    # 加载游戏配置
    if config_path is None:
        config_path = f"config/rules/{game_name.replace(' ', '_').lower()}.yaml"

    # 创建适配器
    if game_name.lower() in ('melvor_idle', 'melvor idle', 'melvor'):
        adapter = MelvorIdleAdapter({
            'name': 'Melvor Idle',
            'url': 'https://melvoridle.com/index_game.php'
        })
    else:
    ...
    # 创建引擎
    diagnosis_engine = DiagnosisEngine(adapter)
    planning_engine = PlanningEngine(adapter)
    decision_engine = DecisionEngine(adapter)
    ...
    async def diagnosis_task():
        ...
        log(f'[诊断] 瓶颈: {result.bottlenecks} | 警告: {result.warnings} | 建议: {result.recommendations}')
...
def main():
    ...
    parser.add_argument('--mode', '-m', default='auto', choices=['auto', 'inspect', 'guards', 'manual'], help='运行模式')
